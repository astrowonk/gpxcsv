...
class GpxCSV:
    ...
    def _process_track(self, trk):
        ...
        non_trkseg_dict = {
            x.tag: x.text
            for x in [x for x in list(trk) if x.tag not in ('trkseg', 'link', 'extensions')]
        }
        ...
        if trk.find('extensions') is not None:
            for extension in list(trk.find('extensions')):
                if extension.getchildren():
                    ...
                    non_trkseg_dict.update({extension.tag: _try_to_float(extension.text)})

        self._check_verbose_print(f'Processing trk with tag info {non_trkseg_dict}')
    ...
    def _list_to_json(self, list_of_dicts, json_file):
        ...
        if not list_of_dicts:
            print('No valid data to convert to json. Please examine the gpx file directly.')
            return
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(list_of_dicts, f, ensure_ascii=False, indent=4)
        self._check_verbose_print(
            f'JSON written to {json_file} with {len(list_of_dicts)} records.', True
        )

    def _list_to_csv(self, list_of_dicts, csv_file):
        """Process a list of dictionaries into a csv file"""
        if not list_of_dicts:
            print('No valid data to convert to csv. Please examine the gpx file directly.')
            return
        header = {}
        ...
        with open(csv_file, 'w') as f:
            mywriter = csv.DictWriter(f, fieldnames=header, quoting=csv.QUOTE_MINIMAL)
            mywriter.writeheader()
            for row in list_of_dicts:
                mywriter.writerow(row)
        self._check_verbose_print(
            f'CSV {csv_file} written to {csv_file} with {len(list_of_dicts)} rows', force=True
        )
        if self.verbose:
    ...
    def gpxtolist(self, gpxfile):
        ...
        if (
            self.errors == 'ignore'
            and isinstance(gpxfile, str)
            and not (gpxfile.endswith('.gpx') or gpxfile.endswith('.gpx.gz'))
        ):
            return []
        assert (
...
def gpxtofile(*args, verbose=False, errors='ignore', **kwargs):
